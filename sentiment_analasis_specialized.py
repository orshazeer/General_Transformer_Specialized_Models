...
class Preprocessing:
    ...
    def preprocessing(self):
        ...
        a = open('sentiment_data.txt','w')
        z = 0
        total_bytes = 0
        for i in dataset1:
            if z < 800000:
                total_bytes += len(i['text'])
            a.write(i['text'])
            lines += 1
    ...
    def data_preperation(self):
        ...
        with open('tokenized_sentiment.bin', mode='wb') as file:

            with open('sentiment_data.txt', "r", encoding="utf-8") as input_file:
                bytes_read = 0
    ...
    def fetch_training_batch_old_version(self, tokens_per_batch, step_num):
        ...
        file = open('tokenized_sentiment.bin','rb')
        total_bytes = os.path.getsize('tokenized_sentiment.bin')
    ...
    def fetch_training_batch(self, B, S):
        ...
        for b in range(B):
            ...
            if start_byte < magic_number:
                pos_or_neg.append(0)
            elif start_byte >= magic_number:
                pos_or_neg.append(1)
            file.seek(start_byte)
...
class Transformer(torch.nn.Module):
    ...
    def make_batch(self, step, istraining):
        ...
        if istraining:
            ...
            input_token_BS, targets = self.preprocessing.fetch_training_batch(B, S)
            # input_token_BS = torch.cat(
            #     (torch.full((B, 1), start_token), target_token_BS[:, :-1]), dim=1
            # )
            return input_token_BS, targets         
    ...
    def training_step(self,i,writer,training,input_sentence, lr):
        ...
        if training:
            ...
            targets_B = torch.tensor(targets_BS)
            targets_B = targets_B.to(self.device).float()

            # print(targets_B)
            # for i in inputs_BS.tolist():
            #     print(self.tokenizer.convert(i))
        ...
        if training:
            logits_B = torch.mean(logits_BSV, dim=(1,2))
        else:
            logits_B = torch.mean(logits_BSV, dim=(0,2))
        # print(logits_B)
        logits_sigmoid = torch.nn.functional.sigmoid(logits_B) 
        logits_sigmoid_2 = torch.mean(logits_sigmoid, dim=0).item()
        if logits_sigmoid_2 >= 0.5:
            pn = "positive"
        elif logits_sigmoid_2 < 0.5:
            pn = "negative"
        # print("logits",logits_sigmoid)
        
        # logits_flat = logits_BSV.reshape(-1, logits_sigmoid.size(-1)).to(self.device)  # Reshape to (Batch size * Sequence Length, Vocab size)\
        
        # if training:
        #     target_flat = targets_B.to(torch.int64).reshape(-1).to(self.device)
        ...
        # print(targets_B.shape)
        loss_B = torch.nn.functional.binary_cross_entropy(logits_sigmoid, targets_B)
    ...
    def train_loop(self):
        ...
        save_interval = 1000
        saved_model_path = "Models/model.sentiment1_515000"
        
        write_model_path = "Models/model.sentiment1_"+str(i)
...
# Close the writer when done
    ...
    def sample(self,a):
        ...
        saved_model_path = "Models/model.sentiment1_516000"
        if os.path.exists(saved_model_path):
            self.load_state_dict(torch.load(saved_model_path))
            torch.load(saved_model_path)
        sentence_length = 25
        tokens1 = self.tokenizer.tokenize_string(a)
